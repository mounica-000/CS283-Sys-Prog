...
int boot_server(char *ifaces, int port){
    ...
    // DONE set up the socket - this is very similar to the demo code
...
int process_cli_requests(int svr_socket){
    ...
    int     rc = OK; 
    int recv_size;   
    // uint8_t send_buffer[RDSH_COMM_BUFF_SZ];
    // uint8_t recv_buffer[RDSH_COMM_BUFF_SZ];
    int is_eof;
...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        io_size = 0;
        while((recv_size = recv(cli_socket, io_buff +io_size, RDSH_COMM_BUFF_SZ-io_size, 0)) > 0) {
            io_size += recv_size;
            if (recv_size < 0) {
                perror("read error");
                exit(EXIT_FAILURE);
            ...
            if (recv_size == 0) {
                //received nothing - break or return
                break;
            }

            is_eof = ((char)io_buff[io_size-1] == eof_char) ? 1 : 0;
        ...
        char* cmd_copy;
        cmd_copy = (char*)malloc(strlen(io_buff) + 1);
        if (cmd_copy == NULL) {
            free(io_buff);
            return ERR_MEMORY;
        }
        strcpy(cmd_copy, io_buff);
        ...
        if (result != NULL) { // pipe exists
            ...
            while (token != NULL) {
                ...
                cmd_list.commands[count-1] = command;
        ...
        //     for (j = 0; j < cmd_list.commands[i].argc; j++) {
        //         printf("'%s', ", cmd_list.commands[i].argv[j]);
        //     }
        //     printf("| \n");
        // }
        // printf("\n");
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    if (clist->num == 1) {
        ...
        if (bi_cmd != BI_NOT_BI) {
            if (send_message_eof(cli_sock) != OK) {
                // free_cmd_list(&cmd_list);
                return ERR_RDSH_COMMUNICATION;
            }
            return OK;
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pids[i] == 0) {  
            ...
            if (i == clist->num - 1) {
                int redirection_present = 0;
                for (int m = 0; m < clist->commands[i].argc -1; m++) {
                    if (strcmp(clist->commands[i].argv[m], ">") == 0) {
                ...
                // If no redirection, write to socket
                if (!redirection_present) {
