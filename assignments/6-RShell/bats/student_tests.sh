...
@test "No arguments does not crash" {
    run ./dsh <<EOF                
...
# @test "Dragon command behaves as intended" {
#     run "./dsh" <<EOF                
# dragon
# EOF
...
#     # Expected output with all whitespace removed for easier matching
#     expected_output="localmodedsh4>Notimplementedyetdsh4>cmdloopreturned0"
...
@test "ls command works with pipe" {
    run "./dsh" <<EOF               
ls | grep .c
EOF
...
@test "Pipe with 2 commands and 2 args each" {
    run "./dsh" <<EOF                
ls bats | wc -l
EOF
...
@test "Pipe with 3 commands and 2 args each" {
    run "./dsh" <<EOF                
cat dshlib.h | grep typedef | wc -l
EOF
...
@test "Input with more than CMD_MAX prints error message" {
    run "./dsh" <<EOF                
