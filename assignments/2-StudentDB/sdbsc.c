...
int get_student(int fd, int id, student_t *s)
{
    ...
    int read_status = read(fd, s, STUDENT_RECORD_SIZE);
    if (read_status < 0) {
        return ERR_DB_FILE;
    } 
    if (s == NULL) {
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    // printf("offset for adding: %ld\n", myoffset);
    // printf("Adding student with id %d at offset: %ld\n", id, myoffset);
    ...
    // check if the student already exists with the same id
    student_t student_at_offset = {0};
    int get_status = get_student(fd, id, &student_at_offset);
    if (get_status == ERR_DB_FILE) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    if (get_status == NO_ERROR) {
        // int res = memcmp(student_at_offset, EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE);
        // if (res != 0) {
    ...
    if (bytes_written < 0) // any other condition to check?
    {
...
int del_student(int fd, int id)
{
    ...
    int result = get_student(fd, id, &student_to_del);
    // print_student(&student_to_del);
...
int count_db_records(int fd)
{
    ...
    int offset = 1 * STUDENT_RECORD_SIZE;
    ...
    while((read_status = read(fd, &student, STUDENT_RECORD_SIZE)) != 0) {
        if (read_status < 0) {
            printf(M_ERR_DB_READ);
            return ERR_DB_FILE;
        }
        if (read_status > 0) {
            int res1 = memcmp(&(student.id), &(EMPTY_STUDENT_RECORD.id), 4);
            int res2 = memcmp(&(student.gpa), &(EMPTY_STUDENT_RECORD.gpa), 4);
            int res3 = memcmp(&(student.fname), &(EMPTY_STUDENT_RECORD.fname), 24);
            int res4 = memcmp(&(student.lname), &(EMPTY_STUDENT_RECORD.lname), 32);
...
int print_db(int fd)
{
    ...
    if ((read_status == 0) && (header_not_printed == 1)) {
        printf(M_DB_EMPTY);
        return NO_ERROR;
    }
